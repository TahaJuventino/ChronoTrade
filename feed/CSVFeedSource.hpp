...
public:
    ...
    void run() override {
        ...
        while (!stop_flag_.load(std::memory_order_acquire) && std::getline(stream_, line)) {
            ...
            double price = 0.0, amount = 0.0;   // initialize to satisfy -Wmaybe-uninitialized
            int64_t ts = 0;
    ...
    void stop() override { stop_flag_.store(true, std::memory_order_release); }

    std::string source_tag() const override { return "SRC_CSV_" + filename_; }
    ...
    FeedTelemetry& telemetry() override { return telemetry_; }
    const FeedTelemetry& telemetry() const override { return telemetry_; }
    FeedStamina& stamina() { return stamina_; }
    const FeedStamina& stamina() const { return stamina_; }

private:
    // Match declaration order to constructor init list to avoid -Wreorder
    ...
    bool parse_line(const std::string& line, double& price, double& amount, int64_t& timestamp) {
        if (!is_ascii_printable(line)) return false;
        ...
        while (std::getline(ss, token, ',')) {
            ...
            if (fields.size() > 3) return false; // reject trailing junk early
        }
    ...
    static bool is_ascii_printable(const std::string& line) {
